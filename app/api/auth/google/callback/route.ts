...
import { getUserByEmail, createMoodleUser, enrollUser } from '@/lib/moodle'
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!tokenData.access_token) {
      return NextResponse.redirect(new URL('/login?error=token_failed', req.nextUrl))
    }
    ...
    if (!email) {
      return NextResponse.redirect(new URL('/login?error=no_email', req.nextUrl))
    }
    ...
    // Create session
    await createSession(moodleUser.id, moodleUser.fullname || name || email, email)
